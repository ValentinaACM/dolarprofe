...
// Define una función asíncrona llamada obtenerDatoApi
const obtenerDatoApi = async () => {
  // Declara la URL de la API de datos sobre el dólar en Colombia
  const urlApiDolar = "https://www.datos.gov.co/resource/mcec-87by.json";

  try {
    // Realiza una solicitud HTTP GET a la URL de la API y espera la respuesta
    const respuestaApi = await fetch(urlApiDolar);

    // Convierte la respuesta de la API (en formato JSON) a un objeto JavaScript y espera la operación
    ...
    // Si se produce un error, imprime en la consola el mensaje "Error consultando API"
    console.log("Error consultando API");
  }
};

// Define una función asíncrona llamada obtenerDatosApi
const obtenerDatosApi = async () => {
  ...
  try {
    ...
    // Obtiene la referencia al elemento HTML con el id "inputPrecioDolar"
    const precioDolar = document.getElementById("inputPrecioDolar");

    // Accede al primer elemento del array de datos obtenido
    const primerElemento = datos[0];

    // Extrae el valor de la propiedad "valor" del primer elemento (si existe)
    const valor = primerElemento && primerElemento.valor;

    // Verifica si el valor obtenido no es indefinido
    if (valor !== undefined) {
      // Asigna el valor al campo de entrada con id "inputPrecioDolar"
      precioDolar.value = valor;
    }
  } catch (error) {
    // Si se produce un error, imprime en la consola el mensaje "Error consultando API"
    console.log("Error consultando API");
